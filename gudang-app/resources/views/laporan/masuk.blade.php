...
<div class="container mt-4">
    <div class="table-wrapper">
        <h4><i class="fas fa-arrow-down me-2"></i> Laporan Barang Masuk</h4>

        {{-- 🔎 Filter tanggal --}}
        <form method="GET" action="{{ route('laporan.masuk') }}" class="filter-bar row g-3 mb-3">
            <div class="col-md-4">
            ...
            <div class="col-md-4 d-flex align-items-end">
                ...
                @if(request('tanggal_awal') || request('tanggal_akhir'))
                    <a href="{{ route('laporan.masuk') }}" class="btn btn-secondary w-50"><i class="fas fa-times"></i> Reset</a>
                @endif
            </div>
        </form>
        ...
        <div class="search-bar mb-3">
            ...
            <form method="GET" action="{{ route('laporan.masuk') }}" style="display: flex; width: 100%;">
                <input type="text" name="search" placeholder="Cari barang masuk..." value="{{ request('search') }}">
                @if(request('search'))
                    <button type="button" onclick="window.location='{{ route('laporan.masuk') }}'" style="background:none;border:none;font-size:18px;margin-left:10px;color:#888;cursor:pointer;" title="Clear Search">&times;</button>
                @endif
            </form>
        </div>
    ...
    @if ($barangMasuk->hasPages())
        <div class="d-flex justify-content-between align-items-center mt-3">
            <div class="text-muted small">
                Menampilkan {{ $barangMasuk->firstItem() ?? 0 }} 
                sampai {{ $barangMasuk->lastItem() ?? 0 }} 
                dari total {{ $barangMasuk->total() }} data
            </div>
            ...
            <div class="pagination-wrapper">
                ...
                @else
                    <a href="{{ $barangMasuk->previousPageUrl() }}" rel="prev">&laquo; </a>
                @endif
                ...
                @if ($barangMasuk->hasMorePages())
                    <a href="{{ $barangMasuk->nextPageUrl() }}" rel="next"> &raquo;</a>
                @else
